...
# Reference: https://github.com/teticio/audio-diffusion/
# Training code for latent diffusion model
...
def main(args):
    ...
    def transforms(examples):
        ...
        if args.encodings is not None:
            encoding = [encodings[os.path.splitext(os.path.basename(file))[0]] for file in examples["audio_file"]]
            print(np.array(encoding).shape)
            encoding = np.array(encoding).reshape(1, 150, 226)
            return {"input": images, "encoding": torch.Tensor(encoding)}
        return {"input": images}
    ...
    for epoch in range(args.num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % args.save_images_epochs == 0:
                ...
                if args.encodings is not None:
                    random.seed(42)
                    encoding = random.sample(list(np.array(list(encodings.values()))), args.eval_batch_size)
                    encoding = np.array(encoding).reshape(args.eval_batch_size, 150, 226)
                    encoding = torch.Tensor(encoding).to(clean_images.device)
                else:
                    encoding = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="aistpp_256_sorted")
    ...
    parser.add_argument(
        "--vae",
        type=str,
        default='teticio/latent-audio-diffusion-ddim-256',
